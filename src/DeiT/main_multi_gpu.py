...
def main_worker(*args):
    ...
    last_epoch = config.TRAIN.LAST_EPOCH  # 用于恢复训练，记录上次训练的epoch，存疑？
    world_size = dist.get_world_size() # 总进程数
    local_rank = dist.get_rank() # 当前进程id
    seed = config.SEED + local_rank
    paddle.seed(seed)
    np.random.seed(seed)
    random.seed(seed) # 初始随机数
    # logger for each process/gpu
    local_logger = get_logger(
            filename=os.path.join(config.SAVE, 'log_{}.txt'.format(local_rank)),
            logger_name='local_logger') # 日志
    # overall logger
    if local_rank == 0:
    ...
    if local_rank == 0:
        master_logger.info(f'----- world_size = {world_size}, local_rank = {local_rank}') # 主进程记录日志
    
    # STEP 1: Create model
    model = build_model(config) # 创建模型
    # define model ema
